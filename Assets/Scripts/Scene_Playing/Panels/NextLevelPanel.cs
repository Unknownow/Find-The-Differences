...
public class NextLevelPanel : MonoBehaviour {
    ...
    public void openNextLevelPanel()
    {
        ...
        int starEarn = gameObject.GetComponent<StarsCount>().getStars() - _playerProperties.getLevelStars(caseIndex, levelIndex);
        Debug.Log("This Level: " + gameObject.GetComponent<StarsCount>().getStars() + " Last time: " + _playerProperties.getLevelStars(caseIndex, levelIndex));
        if (starEarn > 0)
        {
            _livesManager.addGold(starEarn * gameObject.GetComponent<LevelManager>().getBonusGoldsEachStar());
        }
        ...
        _endGameText.GetChild(1).GetComponent<UILabel>().text = "Bonus golds: " + (starEarn * gameObject.GetComponent<LevelManager>().getBonusGoldsEachStar()).ToString();
        _endGameText.GetChild(2).GetComponent<UILabel>().text = "Hints: " + gameObject.GetComponent<LevelManager>().getSpentHints().ToString();
        gameObject.GetComponent<LevelManager>().blurBackground(1);

        

        //_livesManager.addGold(gameObject.GetComponent<LevelManager>().getBonusGoldsEachStar());
    ...
    public void onDoubleGoldClick()
    {
        if (!_isDoubleGold)
        {
            ...
            PlayerPrefs.SetInt("Gold", PlayerPrefs.GetInt("Gold") + gameObject.GetComponent<LevelManager>().getBonusGoldsEachStar());
        }
    }
}
